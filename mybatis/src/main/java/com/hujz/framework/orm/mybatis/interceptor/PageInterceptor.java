 ...
 *
 * @Pr：yscm
 * @Desc: 分页拦截器，用于拦截需要进行分页查询的操作，然后对其进行分页处理。
 * 利用拦截器实现Mybatis分页的原理：
...
public class PageInterceptor implements Interceptor {
  ...
  //缓存
     ...
     * @throws SQLException 
     */
    private void setTotalRecord(PageTools pageTools,BoundSql boundSql, final MappedStatement mappedStatement, final Connection connection) throws SQLException {
    	// 拿到当前绑定Sql的参数对象，就是我们在调用对应的Mapper映射语句时所传入的参数对象
